...
**计划日期**：2026-07-21  
**计划状态**：主要闭环已执行。Metrics 清理、Agent Token、Agent/Monitor WebSocket、真实 Agent 采集上报和隔离 MySQL 验收均已完成；HTTPS/WSS 云端部署已验证（2026-08-07），多实例与跨 JVM 连接/事件状态仍未验证。当前校准日期：2026-08-28，当前基线 `main @ 4e4cd86`（历史记录基于旧提交 `617ccd0`）。后端 591/590 是历史日志记录，未在当前 HEAD 重新执行。历史明文 HTTP/IP/命令仅作追溯，禁止执行。
**适用项目**：SuSuMonitor  

...
- 多 JVM 实例部署 / 跨实例事件推送(阶段一已落地 2026-08-17：Redis ticket 共享 `REDIS_ENABLED` 可选启用；注册表/事件广播仍单 JVM 内存态)
- Redis / 分布式锁 / 微服务拆分(Redis ticket 已落地 2026-08-17 可选启用；分布式锁/微服务拆分仅规划,Outbox 模式已为拆分做准备)

...
> 本文档集是围绕 **SuSuMonitor 服务器监控平台** 整理的面试准备材料,内容全部来自仓库代码、开发日志(148 篇)、Bug 记录、验收文档与交接文档,保证与代码事实一致。
>
> 整理日期:2026-09-06（对应当前实际 HEAD `main @ d8eb375`，旧基线 `09b3911`；此前基于旧提交 `617ccd0` 的提交链计划 `ebe96d0`、验收脚本修正 `ebe152e`、验收记录 `617ccd0`，以及本文档集中仍出现的 `4e4cd86` 表述，均属历史校准，当前基线以根 README 顶部快照和 `docs-SuMon/Develop-plans/20260829-RC1单实例生产可靠性收口计划.md` 为准）。后端历史日志 591/590、Web Vitest 133、Android 85/89 等数字为历史批次或静态统计，当前 HEAD/工作区需在 RC1 干净提交重新执行后才能作为现状；Spring Boot 版本以 `pom.xml`（3.4.13，已提交）为准。历史明文地址、IP 和命令已脱敏并禁止执行；当前部署目标为 HTTPS/WSS，部署边界为单 JVM。

...
| 监控快照面 | 进程快照（v1.4）与磁盘/网卡资源快照（v1.5）三端已通：Agent 采集→`metrics.update` 捎带→REST latest（90 秒内存新鲜窗口、不落库）；后端 **853/853**、Agent 容器 test/vet 全绿；`verify-resources.mjs` 六路真实连跑待联调窗口。 |

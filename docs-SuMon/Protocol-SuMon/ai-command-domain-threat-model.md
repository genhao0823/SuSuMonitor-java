...
## 八、自动审批模式（v2 增补，2026-09-10）

**范围**：实例级自动审批策略（V33 单行表 `ai_command_auto_approval_policies`，默认禁用）允许"AI 来源、风险等级 ≤ 阈值"的建议命令在创建后立即由服务端自动审批并下发，跳过人工点击。仅此一处放宽；其余全部防线不变。

**T4（审批绕过）在本模式下的补偿控制**：

1. **只放宽"谁点按钮"，不放宽校验链**：模板白名单（Java 双端 + Go Agent 独立二次校验）、参数正则、无 shell argv 执行、`server_id` 归属校验、输出脱敏截断全部原样生效。自动审批的命令与人工审批的命令走完全相同的渲染与下发路径。
2. **仅 AI 来源**：自动审批只发生在 `AiCommandSuggestionService` 建议创建流程内部；`source=manual` 的命令永远需要人工审批。
3. **无"审批任意 run"的 REST 端点**：`CommandRunService.autoApprove` 是服务内部方法，不暴露任何路由；不存在把他人 pending 记录改为自动执行的攻击面。
4. **fail-closed**：策略行缺失、读库异常、阈值非法一律视为禁用（行为与 V29 以来完全一致）；部署后默认禁用，需管理员在 Web/Android 页面显式开启。
5. **阈值硬约束**：阈值仅允许 `low` / `medium`；`high` 模板在任何配置下都不可自动审批（`allows()` 按 ordinal 硬比较）。
6. **审计标识**：自动审批行 `approval_mode='auto'`、`approver_id=NULL`，与人工审批（`manual` + 审批人 ID）可区分；`risk_level` 在创建时按模板表快照落审计，模板后续改级不影响历史行。
7. **止损**：UI 随时可关；实例级总熔断 `SUSUMONITOR_AI_COMMAND_ENABLED`（env，默认 false）一键停用整个命令域；既有固定窗口限流（10 次/小时/管理员）对自动审批同样生效。

**新增威胁项**：

| 编号 | 威胁 | 控制 |
|---|---|---|
| T9 | 策略被非管理员篡改（开启自动审批） | 策略端点 `hasRole(ADMIN)`（SecurityConfig `/api/ai/commands/**` 统一声明）；写操作记录 `updated_by` 审计。 |
| T10 | 自动审批失败（如 Agent 离线）静默丢失 | 单条失败置 `failed(40906)` 并保留在建议响应里返回给调用方；不阻断其余建议。 |

**不变式补充**：自动审批不改变 §六 任何既有不变式；AI 仍永不接触 `terminal.*` PTY；命令仍只走 `command.*` 通道。

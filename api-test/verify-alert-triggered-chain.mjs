/**
 * alert.triggered.v1 全链路验收脚本：发布 → 消费 → 通知排程 → 幂等 → DLQ 分类。
 *
 * 验证链（不经 Agent WS / 评估器，直接投递 alert.triggered.v1 信封聚焦消费侧）：
 *   管理 API publish → susumonitor.events -- alert.triggered.v1 --> susumonitor.alert.triggered
 ...
 *
 * 检查项：
 *   P0 准备：管理员登录、服务器、告警规则（cpu>80 + webhook 渠道）
 *   C1 正常消费：合法信封 → 业务队列归零 + alert_notifications 出现 pending 行
 *   C2 重复投递幂等：同 event_id 重投 → 通知行数不变（无第二次排程）
 *   C3 DLQ 分类：非法 JSON / schema_version=2 / 非法 metric → alert.triggered.dlq 增量 ≥3
 *   C4 失败留痕：message_consume_records 存在 failed 行（consumer=alert-notifier）
 */
...
const QUEUE = 'susumonitor.alert.triggered'
const QUEUE_DLQ = 'susumonitor.alert.triggered.dlq'
const ROUTING_KEY = 'alert.triggered.v1'
...
/** 构造符合冻结契约 §四 的 alert.triggered.v1 信封。 */
...
// ---- P0 准备：管理员 + 服务器 + 告警规则（含 webhook 渠道） ----
...
// 真实链路中 record 由评估器先创建再发布事件；这里模拟该前提：插入一条告警记录。
// （独立连接分两步执行：INSERT 后用唯一标记定位 id，避免多语句引号解析问题。）
const recordToken = `rmq-accept-${suffix}`
mysqlExec(
  `INSERT INTO alert_records (rule_id, server_id, metric, current_value, threshold_value, level, status, message, triggered_at)
    VALUES (${ruleId}, ${serverId}, 'cpu', 92.5, 80.0, 'warning', 'unread', '${recordToken}', UTC_TIMESTAMP())`)
const recordIdC1 = mysqlScalar(`SELECT id FROM alert_records WHERE message='${recordToken}' LIMIT 1`)
check('P0', recordIdC1 !== null && Number(recordIdC1) > 0, `告警记录插入（record_id=${recordIdC1}）`)

// ---- C1 正常消费：发布合法信封 → 队列归零 + 通知行出现 ----
...
await waitForCondition(async () => (await queueMessages(QUEUE)) === 0, 'susumonitor.alert.triggered 队列归零')
...
check('C1', notificationsC1 >= 1, `消费驱动通知排程落库（${notificationsC1} 行）`)

const consumeRowC1 = await waitForCondition(async () => {
  const row = mysqlScalar(
    `SELECT status FROM message_consume_records WHERE consumer='alert-notifier' AND event_id='${eventIdC1}'`)
  return row ?? null
}, 'alert-notifier 幂等记录落库')
...
}, 'alert.triggered.dlq 增量 ≥3')
check('C3', dlqAfter >= dlqBefore + 3, `三条坏消息全部进 DLQ（${dlqBefore} → ${dlqAfter}）`)

// ---- C4 失败留痕：alert-notifier 存在 failed 行 ----
const failedRows = await waitForCondition(async () => {
  const row = mysqlScalar(
    `SELECT COUNT(*) FROM message_consume_records WHERE consumer='alert-notifier' AND status='failed'`)
  return row !== null && Number(row) >= 1 ? Number(row) : null
}, 'alert-notifier failed 留痕行')
...
console.log('\nDB 人工确认指引：')
console.log(`  SELECT event_id,status FROM message_consume_records WHERE consumer='alert-notifier' ORDER BY id DESC LIMIT 6;`)

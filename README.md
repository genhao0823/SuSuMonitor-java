...
| 当前基线 | `fix/web-glassmorphism-closeout`（代码基线 `617ccd0`，当前 PR 分支；合并目标为 `main`） |
...
> 本节反映 2026-08-27 的当前仓库基线；下方带日期的“对齐说明”和“收口”段落均为历史记录（原文保留，当前值以本快照为准）。历史命令和临时公网配置均禁止直接执行。
...
│       └── util/           # 格式化/错误码/常量
├── docs-SuMon/              # 项目文档
│   ├── Develop-log/         # 142 篇 dev-log(实施记录)
│   ├── Develop-plans/       # 31 个 plan(规划)
│   ├── OpenApi-SuMon/       # 5 个 OpenAPI 契约 JSON(auth / server / system / admin / alert),共 31 路径 / 36 端点操作
...
├── api-test/                # API 集成测试(Apifox + curl + WS 验证,23 个 mjs + 4 个 ps1)
...
- **OpenAPI 契约**: `docs-SuMon/OpenApi-SuMon/{openapi-auth,server,system,admin,alert}.json`（5 个文件 / 31 路径 / 36 个端点操作）

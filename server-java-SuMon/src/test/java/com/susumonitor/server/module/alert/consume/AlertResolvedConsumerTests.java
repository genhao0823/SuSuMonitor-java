...
/**
 * AlertResolvedConsumer 单元测试（全 mock，不依赖 Broker/DB）。
 *
 * <p>AUTO 确认模式下 ACK 由容器在监听方法正常返回后执行，本测试只断言
 * 业务行为（恢复通知排程 / 幂等记录插入 / 异步发送 / 不可重试异常分类）。</p>
 */
class AlertResolvedConsumerTests {
    ...
    private AlertResolvedConsumer consumer;
    ...
    void setUp() {
        ...
        consumer = new AlertResolvedConsumer(objectMapper, notificationService, ruleMapper, recordMapper,
                consumeRecordMapper, transactionTemplate,
                Clock.fixed(Instant.parse("2026-08-15T12:00:06Z"), ZoneOffset.UTC),
                new AlertResolvedMessageValidator());
    }
    ...
    void idempotentHitSkipsDispatchAndInsert() {
        ...
        when(consumeRecordMapper.existsConsumed(AlertResolvedConsumer.CONSUMER_NAME, EVENT_ID)).thenReturn(true);
    ...
    /** 首次消费（规则有效 + 有渠道 + 记录已恢复）：排程恢复通知 + 幂等记录 + 提交后异步发送。 */
    @Test
    void firstConsumeSchedulesAndSends() {
        AlertRuleEntity rule = activeRuleWithChannel();
        AlertRecordEntity record = resolvedRecord();
        stubRule(rule);
        when(recordMapper.selectRecordById(RECORD_ID)).thenReturn(record);
        when(consumeRecordMapper.existsConsumed(AlertResolvedConsumer.CONSUMER_NAME, EVENT_ID)).thenReturn(false);
        ...
        verify(consumeRecordMapper).upsertConsumed(argThat(r ->
                AlertResolvedConsumer.CONSUMER_NAME.equals(r.getConsumer())
    ...
    /** 记录尚未恢复（非 resolved）：仅幂等记录，不排程不发送（恢复通知语义不成立）。 */
    @Test
    void notResolvedRecordSkipsDispatchButConsumes() {
        stubRule(activeRuleWithChannel());
        AlertRecordEntity record = resolvedRecord();
    ...
    void businessExceptionPropagates() {
        AlertRuleEntity rule = activeRuleWithChannel();
        stubRule(rule);
        when(recordMapper.selectRecordById(RECORD_ID)).thenReturn(resolvedRecord());
        when(consumeRecordMapper.existsConsumed(AlertResolvedConsumer.CONSUMER_NAME, EVENT_ID)).thenReturn(false);
    ...
    private AlertRecordEntity resolvedRecord() {
        ...
        record.setTriggeredAt(LocalDateTime.of(2026, 8, 15, 10, 0, 5));
        record.setResolvedAt(LocalDateTime.of(2026, 8, 15, 11, 30, 5));
        return record;
    }
    ...
    /** 构造符合冻结契约 §五 的信封 JSON（snake_case）。 */

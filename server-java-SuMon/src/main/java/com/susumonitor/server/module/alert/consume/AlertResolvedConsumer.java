...
/**
 * 告警恢复事件消费者（消息驱动"恢复通知"）：幂等消费 alert.resolved.v1。
 *
 * <p>与 {@link AlertTriggeredConsumer} 对称：恢复事务登记 Outbox →
 * 发布器路由到 {@code susumonitor.alert.resolved} → 本消费者在消费事务内
 * 为规则配置的渠道排程恢复通知（同事务插 pending 行 + 消费幂等记录），
 * 事务提交后异步发送（{@link AlertNotificationService#sendScheduled}）。
 * 通知文案由 {@code AlertMessageBuilder} 按记录状态生成"已恢复"语义。</p>
 *
 * <p>ACK 语义与错误分类与 {@link AlertTriggeredConsumer} 一致：AUTO 确认
 * （事务提交后返回即 ACK）；解析/字段契约不符抛
 * {@link AmqpRejectAndDontRequeueException} 直接进 DLQ；业务异常由容器级
 * 有限重试后进 DLQ。</p>
 */
@Slf4j
@Component
...
public class AlertResolvedConsumer {
    ...
    static final String CONSUMER_NAME = "alert-resolved-notifier";

    static final String QUEUE = "susumonitor.alert.resolved";
    ...
    private final AlertResolvedMessageValidator messageValidator;

    /** 注入反序列化器、通知服务、规则/记录数据访问与事务模板。 */
    public AlertResolvedConsumer(ObjectMapper objectMapper, AlertNotificationService notificationService,
            AlertRuleMapper ruleMapper, AlertRecordMapper recordMapper, ConsumeRecordMapper consumeRecordMapper,
            TransactionTemplate transactionTemplate, Clock clock, AlertResolvedMessageValidator messageValidator) {
    ...
    /**
     * 消费一条 alert.resolved.v1 消息。
     *
     * @param message 原始消息（UTF-8 JSON 信封）
     */
    @RabbitListener(queues = QUEUE)
    public void onMessage(Message message) {
        AlertResolvedMessage envelope = parseEnvelope(message);
    ...
    /**
     * 在消费事务内排程恢复通知：规则有效且配置了至少一个渠道、对应记录存在
     * 且已是 resolved 状态时，为每个渠道登记 pending 行；其余情况仅继续消费
     * 幂等记录（不产生通知）。
     *
     * @return 排程结果（规则/记录/待发通知行），供事务提交后异步发送
     */
    private DispatchResult dispatchNotifications(AlertResolvedMessage envelope) {
        AlertResolvedMessage.Payload payload = envelope.payload();
        ...
        if (rule == null || !Boolean.TRUE.equals(rule.getEnabled()) || !hasAnyChannel(rule)) {
            log.debug("alert resolved notification skipped: rule unavailable or has no channel, ruleId={}, recordId={}",
        ...
        if (record == null || !"resolved".equals(record.getStatus())) {
            log.warn("alert resolved notification skipped: record missing or not resolved, recordId={}",
                    payload.recordId());
            return null;
        }
     ...
     */
    private AlertResolvedMessage parseEnvelope(Message message) {
        String body = new String(message.getBody(), StandardCharsets.UTF_8);
        AlertResolvedMessage envelope;
        try {
            envelope = objectMapper.readValue(body, AlertResolvedMessage.class);
        ...
        if (envelope.schemaVersion() != AlertResolvedEnvelopeFactory.SCHEMA_VERSION
                || !AlertResolvedEnvelopeFactory.EVENT_TYPE.equals(envelope.eventType())) {
        ...
        try {
            ...
            log.warn("consume rejected: invalid alert resolved contract");
            throw new AmqpRejectAndDontRequeueException("invalid alert resolved contract", exception);
        }
        return envelope;
    }

    /** 插入消费幂等记录，已存在则翻转为 consumed。 */
    private void insertConsumeRecord(AlertResolvedMessage envelope) {

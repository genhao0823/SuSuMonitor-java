...
public class AuthController {
     ...
     * 防止批量注册淹没审核队列与数据库。批次 8 起：首管理员未初始化时
     * 必须携带有效一次性初始化令牌（缺失 40310 / 不匹配 40311），
     * 令牌消费与首管理员创建同事务完成。</p>
     *
    ...
    @Operation(
            ...
            description = "Registers a user. When the system still awaits its first admin "
                    + "(bootstrapPending=true on GET /api/auth/bootstrap-status), a valid one-time "
                    + "bootstrap token must be provided in bootstrapToken; the token is consumed on "
                    + "success and the first user becomes admin/approved. Missing token returns "
                    + "403/40310 and an invalid token returns 403/40311. After the first admin exists, "
                    + "subsequent users become user/pending and bootstrapToken is ignored. "
                    + "X-Request-ID is generated by the server; "
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "403",
                    description = "Bootstrap token required (40310) or invalid (40311) "
                            + "while the system still awaits its first admin"),
    ...
    /**
     * 查询系统是否仍待初始化首管理员（批次 8 公开端点）。
     *
     * <p>三端（Web/Android/契约方）据此决定注册页是否展示初始化令牌输入框。
     * 仅暴露一个布尔值，无认证要求；令牌本身的投递渠道是启动横幅或
     * AUTH_BOOTSTRAP_TOKEN 环境变量，本端点不涉及令牌值。</p>
     *
     * @return 首管理员初始化状态
     */
    ...
    @Operation(
            ...
            description = "Public endpoint returning whether the system still awaits its first admin. "
                    + "When bootstrapPending is true, POST /api/auth/register requires a valid "
                    + "one-time bootstrap token (40310 when missing, 40311 when invalid); the token "
                    + "is delivered to operators via the server startup log banner or the "
    ...
    public com.susumonitor.server.common.ApiResponse<
            com.susumonitor.server.module.auth.vo.BootstrapStatusVo> bootstrapStatus() {
        return com.susumonitor.server.common.ApiResponse.success(
                new com.susumonitor.server.module.auth.vo.BootstrapStatusVo(
                        userService.getBootstrapPending()));
    }

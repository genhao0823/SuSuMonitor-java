...
/**
 * alert.triggered.v1 字段级契约校验器。
 *
 * <p>只校验消息可在通知分发侧安全消费所需的不变量（信封、身份、契约枚举集）；
 * 失败由消费方分类为不可重试数据错误。不记录原始消息内容，避免日志泄漏未经审查的 payload。</p>
 */
@Component
public class AlertTriggeredMessageValidator {

    /** 契约生产模块标识（与 AlertTriggeredEnvelopeFactory 同源）。 */
    private static final String PRODUCER = AlertTriggeredEnvelopeFactory.PRODUCER;

    /** 冻结指标名集合（message-contracts-v1.md §四 字段规则）。 */
    private static final Set<String> FROZEN_METRICS = Set.of(
            "cpu", "memory", "disk", "temperature", "load_avg");

    /** 校验冻结的 alert.triggered.v1 信封及载荷。 */
    public void validate(AlertTriggeredMessage message) {
        ...
        if (payload.metric() == null || !FROZEN_METRICS.contains(payload.metric())) {
            throw new IllegalArgumentException("invalid alert event metric");
        }
        if (payload.currentValue() == null || payload.thresholdValue() == null
                || payload.level() == null || payload.level().isBlank()
                || payload.status() == null || payload.status().isBlank()
                || !isUtcOffsetDateTime(payload.triggeredAt())) {
            throw new IllegalArgumentException("invalid alert event values");
        }
    }
